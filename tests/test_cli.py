...
def run_cli(argv, stdin_payload: str | None = None, *, patch_search=None):
    ...
    try:
        if patch_search is not None:
            patched["search"] = patch(
                "MyMovieGraphQL.Search.search", return_value=patch_search
            )
...
def test_cli_valid_search_arguments_and_output():
    result = DummyResult(
        {"__typename": "Title", "id": "tt0000001", "titleText": "Example"}
    )
    _, output, _ = run_cli(
...
def test_cli_name_and_title_variants_parse_expected_arguments():
    ...
    for argv in (
        ...
        with patch("MyMovieGraphQL.Search.searchName", return_value=result), patch(
            "MyMovieGraphQL.Search.searchTitle", return_value=result
        ):
            run_cli(argv)
    ...
    name_fn = next(
        node
        for node in cli_tree.body
        if isinstance(node, ast.FunctionDef) and node.name == "nameSearch"
    )
    title_fn = next(
        node
        for node in cli_tree.body
        if isinstance(node, ast.FunctionDef) and node.name == "titleSearch"
    )
    # fmt: off
...
def test_cli_type_contract_and_expected_io_shape():
    cli_tree = ast.parse(CLI_PATH.read_text(encoding="utf-8"))
    defs = {
        node.name: node for node in cli_tree.body if isinstance(node, ast.FunctionDef)
    }
